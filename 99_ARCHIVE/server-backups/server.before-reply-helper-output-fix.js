...
app.post('/api/luna', async (req, res) => {
  ...
  // ── WhatsApp Helper — separate prompt and response shape ─────────────────
  if (detectedType === 'whatsapp') {
    const waPrompt =
      `You are Luna, a friendly bilingual assistant for Spanish-speaking English learners. ` +
      `The learner pasted a WhatsApp message they received (probably in English) and needs help understanding it and replying. ` +
      `Reply with ONLY a single valid JSON object. No markdown. No code fences. Begin with { end with }. ` +
      `Required keys — keep every value short and beginner-friendly: ` +
      `"meaning" (explain what the message means in Spanish, 1-2 simple sentences), ` +
      `"casualReply" (a natural casual English reply, 1-2 sentences — sounds like a real friend), ` +
      `"politeReply" (a polite respectful English reply, 1-2 sentences), ` +
      `"shortReply" (the shortest possible English reply, ideally under 8 words — easy to copy and send), ` +
      `"note" (one short Spanish coaching tip about English usage in this message, under 15 words — e.g. "En inglés 'free' puede significar disponible, no gratis.").`;

    try {
      const waRes = await fetch('https://api.anthropic.com/v1/messages', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-api-key': ANTHROPIC_API_KEY, 'anthropic-version': '2023-06-01' },
        body: JSON.stringify({ model: 'claude-haiku-4-5-20251001', max_tokens: 350, system: waPrompt, messages: [{ role: 'user', content: input.trim() }] })
      });
      ...
      if (!waParsed) waParsed = { meaning: waStr.slice(0, 120), casualReply: '', politeReply: '', shortReply: '', note: '' };

      return res.json({
        mode:         'whatsapp',
        meaning:      waParsed.meaning      || '',
        casualReply:  waParsed.casualReply  || '',
        politeReply:  waParsed.politeReply  || '',
        shortReply:   waParsed.shortReply   || '',
        note:         waParsed.note         || '',
        detectedLang
      });
    } catch (err) {
