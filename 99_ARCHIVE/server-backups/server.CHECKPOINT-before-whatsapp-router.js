...
app.post('/api/luna', async (req, res) => {
  ...
  if (detectedType === 'whatsapp') {
    const waPrompt =
      ...
      `\n1. suggestedReplyEnglish is the learner's ANSWER to the friend. It must never be a correction or rephrasing of the friend's message.` +
      `\n2. If the friend's message has grammar mistakes, DO NOT put the corrected version as the reply. Fix it only in a small note if useful.` +
      ...
      `\nFriend: "How is you morning going?" → suggestedReplyEnglish: "It's going well, thank you! How about yours?" (do NOT reply: "How is your morning going?")` +
      ...
      `\n\nIf the input is Spanish (e.g. "Cómo digo que estoy cansada?"), the learner is asking how to say something. ` +
      `Generate the natural English phrase they want to express as suggestedReplyEnglish.` +
      ...
      `\n- friendMeaning: one short Spanish sentence explaining what the friend said (e.g. "Tu amigo te saluda y pregunta cómo estás.")` +
      `\n- suggestedReplyEnglish: the learner's natural reply to the friend — never a repeat or correction of the friend's words` +
      `\n- suggestedReplySpanish: Spanish translation of suggestedReplyEnglish` +
      `\n- note: one short Spanish grammar or vocab tip (under 12 words), or empty string`;
