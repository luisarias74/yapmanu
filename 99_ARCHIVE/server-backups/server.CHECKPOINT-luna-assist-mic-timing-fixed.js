...
app.post('/api/luna', async (req, res) => {
  ...
  const systemPrompt =
    ...
    `IMPORTANT: Reply with ONLY valid minified JSON. No markdown. No code fences. No explanation outside the JSON object. ` +
    ...
    `Keep every value to 1\u20132 lines. Be calm and concise.`;
  ...
  try {
    ...
    try {
      ...
      // 2. Try extracting first { ... last } block (handles prose wrapping)
      const start = rawTrimmed.indexOf('{');
      const end   = rawTrimmed.lastIndexOf('}');
      if (start !== -1 && end > start) {
        try {
          parsed = JSON.parse(rawTrimmed.slice(start, end + 1));
        } catch (_2) {
          parsed = null;
        }
      }
    ...
    // 3. Graceful fallback — return raw text as english field
    if (!parsed) {
      console.warn('[Luna proxy] Could not parse model JSON, using fallback. raw:', rawTrimmed.slice(0, 120));
      parsed = {
        english: rawTrimmed.slice(0, 120),
        meaning: '',
        example: '',
        note:    null
      };
